...
const WORKBUDDY_DISCOVERY_BUDGET_MS = 1e4;
...
*/ var WorkBuddyAtRestKeyProvider = class {
	...
	*/
	...
	constructor(options = {}) {
		...
		this.defaultPath = options.defaultElectronPath === void 0 ? defaultWorkBuddyElectronPath() : options.defaultElectronPath ?? void 0;
	...
	*/
	async discoverMacosApp() {
		if (process.platform !== "darwin") throw new WorkBuddyElectronPathError("electron-binary-unavailable", `no WorkBuddy Electron binary is configured for this platform; set ${WORKBUDDY_ELECTRON_BIN_ENV} to the app's Electron binary`);
		const controller = new AbortController();
