...
*/
function defaultWorkBuddyElectronPath() {
	return process.platform === "darwin" ? MACOS_ELECTRON_PATH : void 0;
}
...
var WorkBuddyAtRestKeyProvider = class {
	...
	constructor(options = {}) {
		...
		this.electronPath = options.electronPath ?? (fromEnv !== void 0 && fromEnv !== "" ? fromEnv : void 0) ?? defaultWorkBuddyElectronPath();
	...
	*/
	async protectorKeyFor(requested) {
		...
		if (!requested.includes(resolved.keyId)) throw new Error(`WorkBuddy's current at-rest key (id ${resolved.keyId}) does not match the credential's envelope (id ${requested.join(" or ")}); the desktop credential was sealed by a different WorkBuddy installation`);
		return resolved.key;
	}
	ingest(text) {
		const payload = parseAtRestPayload(text);
		if (payload === void 0) throw new Error("WorkBuddy key helper returned an unusable at-rest payload (expected {version:1, atRestSecretKey})");
	...
	async spawnPayload() {
		...
		if (electronPath === void 0) throw new Error(`no WorkBuddy Electron binary is known for ${process.platform}; set ${WORKBUDDY_ELECTRON_BIN_ENV} to the app's Electron binary`);
		try {
		...
		return await new Promise((resolve, reject) => {
			execFile(electronPath, [HELPER_SCRIPT_ARGUMENT_FLAG, HELPER_SCRIPT], {
				...
				if (error !== null && error !== void 0) {
					...
					reject(/* @__PURE__ */ new Error(`the WorkBuddy key helper (${electronPath}) ${reason}`));
					return;
				}
				const output = stdout.trim();
				if (output === "") {
					reject(/* @__PURE__ */ new Error(`the WorkBuddy key helper (${electronPath}) produced no payload`));
					return;
				}
				resolve(output);
			});
	...
	*/
	...
	async openEncryptedDesktop(classification) {
		...
		return parseWorkBuddyAuth(unwrapDesktopAuthDocument(classification, (field) => {
			...
			if (plaintext === void 0) throw new Error(`the encrypted desktop credential's ${field.field} could not be decrypted (envelope key id ${field.envelope.keyId}); the WorkBuddy app may hold a different at-rest key — open it once to reseal the sign-in`);
			return plaintext;
		}));
	}
	/**
...
const WORKBUDDY_CONNECT_VERSION = "0.6.0";
//#endregion
...
//#endregion
export { normalizeCredits as A, chatUserAgent as B, desktopAuthCandidatesFor as C, WorkBuddyUpstreamClient as D, WorkBuddyAtRestKeyProvider as E, PROBE_EFFORT_CANDIDATES as F, WORKBUDDY_APP_VERSION_FILENAME as G, readCliVersion as H, probeModel as I, readBundleVersion as J, appUserAgent as K, randomSentinel as L, prepareChatBody as M, prepareInternationalChatBody as N, classifyUpstreamError as O, regionOf as P, CN_APP_VERSION_FILENAME as R, defaultDesktopAuthPath as S, workbuddyOwnAuthPath as T, resolveChatIdentity as U, fallbackChatIdentity as V, validCliVersion as W, validAppVersion as X, resolveAppVersion as Y, WorkBuddyCatalog as _, WORKBUDDY_HOST_HEARTBEAT_FILENAME as a, WorkBuddyCredentialStore as b, processStartTimeMs as c, writeHostHeartbeat as d, WORKBUDDY_CONNECT_VERSION as f, FALLBACK_WORKBUDDY_MODELS as g, FALLBACK_WORKBUDDY_AI_MODELS as h, variantFor as i, parseModelCatalog as j, modelWithCurrentPromotion as k, readHostHeartbeat as l, WORKBUDDY_STATUS_PATH as m, CN_VARIANT as n, clearHostHeartbeat as o, WORKBUDDY_PROBE_PATH as p, installedAppVersion as q, WORKBUDDY_VARIANTS as r, isHeartbeatProcessAlive as s, AI_VARIANT as t, workbuddyHostHeartbeatPath as u, WORKBUDDY_AUTH_FILENAME as v, parseWorkBuddyAuth as w, defaultDesktopAuthCandidates as x, WORKBUDDY_AUTH_FILE_ENV as y, FALLBACK_CN_APP_VERSION as z };
